...
export function HomePage() {   
  ...
  const nextSlide = () => setIndex((prev) => (prev + 1) % carouselData.length);
  const prevSlide = () => setIndex((prev) => (prev - 1 + carouselData.length) % carouselData.length);
  ...
  return (
    <>
      ...
      <DefaultTopBar />
      <div className="pt-14 min-h-screen bg-neutral-50 text-neutral-800 font-sans">
        
        <div className="relative flex flex-col justify-center h-[70vh] overflow-hidden w-full bg-neutral-950 shadow-inner">
          
          <div 
            className="absolute inset-0 flex w-full h-full transition-transform duration-700 ease-in-out z-0"
          ...
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 z-20 flex flex-row w-full justify-between items-center px-6 pointer-events-none">
            <button className="cursor-pointer pointer-events-auto transition-transform hover:scale-105 active:scale-95" onClick={prevSlide}>
            ...
            <button className="cursor-pointer pointer-events-auto transition-transform hover:scale-105 active:scale-95" onClick={nextSlide}>
              <svg width="50" height="50" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                <circle cx="50" cy="50" r="45" fill="black" opacity="0.4"/>
          ...
          <div className="absolute bottom-6 left-0 right-0 z-20 flex flex-row w-full justify-center gap-2.5">
            {carouselData.map((_, i) => (
              <button key={i} className="cursor-pointer group" onClick={() => setSlide(i)}>
                <div className={`w-3 h-3 rounded-full transition-all duration-300 ${index === i ? "bg-white scale-125" : "bg-white/30 group-hover:bg-white/60"}`} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex py-20 justify-around items-center max-w-7xl mx-auto px-6">
          <div className="flex flex-col w-150 gap-4">
            <span className="text-4xl md:text-5xl font-extrabold text-neutral-900 leading-tight tracking-tight wrap-break-words">
              Sistema de gerenciamento de karaokê
            </span>
            <span className="text-lg md:text-xl text-neutral-500 font-medium leading-relaxed wrap-anywhere">
          ...
          <div className="relative flex items-center justify-center w-36 h-36">
            ...
            <svg viewBox="0 0 24 24" width="24" height="24" className="note-svg-1 absolute text-neutral-400 top-2 left-2 fill-current">
              <path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/>
            </svg>

            <svg viewBox="0 0 24 24" width="28" height="28" className="note-svg-2 absolute text-neutral-500 top-4 right-2 fill-current">
            ...
            <svg viewBox="0 0 24 24" width="20" height="20" className="note-svg-3 absolute text-neutral-400 -top-2 left-14 fill-current">
            ...
            <svg 
              ...
              fill="none" 
              stroke="currentColor" 
              className="text-neutral-900 animate-bounce [animation-duration:3.5s]"
              strokeWidth="2" 
              strokeLinecap="round" 
        ...
        <div className="flex flex-col py-16 min-h-25 bg-neutral-100/60 border-t border-b border-neutral-200">
          <span className="flex text-3xl font-black text-neutral-900 tracking-tight justify-center pb-10">
            Planos
          </span>
        ...
        <div className="flex flex-row w-full bg-neutral-900 text-neutral-100 py-6 px-12 justify-between items-center border-t border-neutral-800">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium text-neutral-400">Contatos:</span>
          </div>
